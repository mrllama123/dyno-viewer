...
from textual.widgets import Footer, Header, OptionList, Markdown
from textual.reactive import reactive
from textual import on, work

from dyno_viewer.components.screens.create_new_table_viewer_session import (
    CreateNewTableViewerSession,
)
...
class TableViewerSessionsSelect(ModalScreen):
    ...
    BINDINGS = [
        Binding("escape", "close_modal", "Close the modal"),
    ...
    async def action_rename_session(self) -> None:
        ...
        if selected_option:
            new_session_name = await self.app.push_screen_wait(
                CreateNewTableViewerSession("Rename Table Viewer Session")
            )
            if new_session_name:
